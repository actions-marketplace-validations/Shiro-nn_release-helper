...
import { getInput, setFailed, info } from "npm:@actions/core";
...
async function assertCleanWorkingDir() {
    ...
    if (output.trim()) {
        throw new Error("Рабочая директория не чиста (unstaged/uncommitted changes)");
    }
}
...
async function assertOnBranch(expected: string) {
    ...
    branch = branch.trim();
    if (branch !== expected) {
        throw new Error(`Релизы разрешены только из ветки '${expected}', текущая ветка '${branch}'`);
    }
}
...
async function getLastReleaseTag(octokit: ReturnType<typeof getOctokit>, owner: string, repo: string): Promise<string|undefined> {
    try {
...
function bumpVersion(lastTag: string|undefined, releaseType?: semver.ReleaseType): string {
    if (!releaseType) throw new Error("Не указан тип релиза для bumpVersion");
    if (!lastTag) {
        // первый релиз
        return releaseType === "major" ? "1.0.0"
...
function validateCommitMessages(commits: Array<{ commit: any }>) {
    const invalid = commits.filter(c => !/^(feat|fix|docs|chore|refactor|style|perf)(\(.+\))?:/.test(c.commit.message));
    if (invalid.length) {
...
async function getAISummary(bullets: string, apiKey: string, model: string, baseUrl: string): Promise<string> {
    const resp = await fetch(baseUrl, {
        ...
        body: JSON.stringify({
            model,
            messages: [
                { role: "system", content: "Ты — помощник, который на основе списка изменений генерирует 2–3 предложения." },
                { role: "user", content: `Вот список изменений:\n${bullets}\n\nСделай короткое резюме.` }
            ]
        })
    });
...
async function deleteTag(octokit: ReturnType<typeof getOctokit>, owner: string, repo: string, tag: string) {
    await octokit.rest.git.deleteRef({ owner, repo, ref: `tags/${tag}` });
}

// Удаление релиза по ID
async function deleteRelease(octokit: ReturnType<typeof getOctokit>, owner: string, repo: string, releaseId: number) {
